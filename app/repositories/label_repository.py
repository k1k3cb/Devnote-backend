...
from app.models.label import Label, NoteLabelLink, LabelShare
...
class LabelRepository:
    ...
    def list_ids_for_owner_subset(self, owner_id: int, ids: list[int]) -> list[int]:
        ...
        query = select(Label.id).where(Label.owner_id == owner_id, Label.id.in_(ids))
        return self.db.exec(query).all()

    def list_label_ids_for_note(self, note_id: int) -> list[int]:
        query = select(NoteLabelLink.label_id).where(NoteLabelLink.note_id == note_id)
        return self.db.exec(query).all()

    def list_note_ids_by_label_ids(self, label_ids: list[int]) -> list[int]:
        if not label_ids:
            return []
        query = select(NoteLabelLink.note_id).where(
