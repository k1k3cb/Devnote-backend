...
class NoteService:
    ...
    def list_visible_notes(self, user_id: int) -> list[Note]:
        ...
        combined = {note.id: note for note in owned + shared}
    ...
    def create_note(self, owner_id: int, payload: NoteCreate) -> Note:
        note = self.notes.create(
            Note(owner_id=owner_id, **payload.model_dump(exclude={"labels_ids"}))
        )
    ...
    def update_note(self, user_id: int, note_id: int, payload: NoteUpdate) -> Note:
        ...
        if not self.user_can_edit(user_id, note):
            raise HTTPException(status_code=403, detail="User cannot edit this note")

        updates = payload.model_dump(exclude_none=True)
        label_ids = updates.pop("labels_ids", None)
